...
AS $function$
      ...
      search_terms IS NULL OR (
        CASE 
          WHEN array_length(search_terms, 1) = 1 THEN
            -- Single term: check for word boundary matches in specified columns
            (
              -- First priority: word boundary match in individual products (comma-separated)
              EXISTS (
              ...
              OR 
              -- Second priority: word boundary match in name
              b.name ~* ('\m' || regexp_replace(search_terms[1], '[^a-zA-Z0-9\s]', '', 'g') || '\M')
              OR
              -- Third priority: word boundary match in category
  ...
  ORDER BY 
    -- Prioritize exact product matches, then name matches, then category matches
    CASE 
      ...
      WHEN search_terms[1] IS NOT NULL AND b.name ~* ('\m' || regexp_replace(search_terms[1], '[^a-zA-Z0-9\s]', '', 'g') || '\M') THEN 3
      WHEN search_terms[1] IS NOT NULL AND b.category ~* ('\m' || regexp_replace(search_terms[1], '[^a-zA-Z0-9\s]', '', 'g') || '\M') THEN 2
      ELSE 1
    END DESC,
