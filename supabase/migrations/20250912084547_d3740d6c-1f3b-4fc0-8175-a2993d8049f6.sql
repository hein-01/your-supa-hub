-- Update search_businesses function to handle improved search logic with better phrase matching
...
AS $$
  ...
  WHERE
    -- Enhanced text search with better phrase prioritization
    (
      search_terms IS NULL OR (
        -- For multi-word searches, require ALL terms to be present (AND logic)
        (
          CASE 
            ...
            ELSE
              -- Multiple terms: ALL must be present somewhere in the business data
              (
  ...
  ORDER BY 
    -- Prioritize exact phrase matches in business name
    CASE 
      ...
      WHEN search_terms[1] IS NOT NULL AND b.products_catalog ILIKE '%' || search_terms[1] || '%' THEN 1
      ELSE 0
    END DESC,
