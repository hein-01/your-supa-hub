-- Drop the existing function and recreate with starting_price
DROP FUNCTION IF EXISTS public.get_public_businesses(text, text, text);

-- Create the updated function that includes starting_price
CREATE OR REPLACE FUNCTION public.get_public_businesses(search_term text DEFAULT NULL::text, category_filter text DEFAULT NULL::text, location_filter text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, name text, description text, category text, city text, state text, rating numeric, image_url text, website text, product_images text[], business_options text[], starting_price text)
 LANGUAGE sql
